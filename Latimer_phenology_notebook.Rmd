...
datphen2 %>% filter(common_name=="BIG TOOTHED ASPEN" & event=="FLOWERING") %>% ggplot(aes(y= julian, x= year)) +
  ...
  labs(title = "Date of Big Toothed Aspen first flower", x = "Year", y = "Julian Calendar Day")
...
datphen2 %>% filter(common_name=="FALSE SOLOMON SEAL" & event=="FLOWERING") %>% ggplot(aes(y= julian, x= year)) +
  ...
  theme(axis.text=element_text(size=12),
        ...
        axis.text.x = element_text(angle = 90, vjust = 0.5),
        legend.text=element_text(size=16)) +
  labs(title = "Date of False Somolon's Seal first flower", x = "Year", y = "Julian Calendar Day")
```
...
looking at the birds -- are htere any trends there?
